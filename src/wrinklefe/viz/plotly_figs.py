"""Interactive Plotly figure builders for FE meshes and result fields.
...
a plotly.graph_objects.Figure. The caller (the Streamlit app, or a
notebook) is responsible for caching the FE arrays and wiring controls
to function arguments.

Plotly is an **optional** dependency (the ``plotly`` / ``streamlit``
extras), so this module is never imported eagerly by
:mod:`wrinklefe.viz`: the package exposes its functions through a
:pep:`562` module ``__getattr__``, and importing ``wrinklefe.viz``
without plotly installed keeps working. Requesting a figure builder
without plotly raises :class:`ImportError` naming the install command.

Examples
--------
>>> from wrinklefe.viz import mesh3d_figure  # doctest: +SKIP
>>> fig = mesh3d_figure(nodes, elements)     # doctest: +SKIP

This module was previously the repository-root ``streamlit_viz`` module;
that name survives as a deprecated re-export shim (issue #286).
"""
...
except ImportError as exc:  # pragma: no cover - exercised in a no-plotly env
    raise ImportError(
        "The interactive Plotly figures require the 'plotly' package. "
        "Install with: pip install 'wrinklefe[plotly]' "
        "(or 'wrinklefe[streamlit]' for the full web-app stack)."
    ) from exc
