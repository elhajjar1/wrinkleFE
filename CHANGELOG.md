...
- Core — **`wrinklefe.core.fit`: fit a `WrinkleProfile` to a measured
  ...
  The wrinkle a user actually has to disposition is a measured one — a
  ply-boundary trace digitized off a polished-section micrograph, or a
  C-scan profile. Until now the amplitude and wavelength that drive
  every downstream number were read off an image by eye, which put an
  unquantified manual step in front of an otherwise traceable pipeline
  and left the choice of morphology to judgement.

  - `fit_profile(x, z, family)` — nonlinear least squares of one family,
    with starting guesses derived from the data (peak-to-peak amplitude,
    FFT dominant period, envelope span). `family="auto"` picks the best.
  - `rank_families(x, z)` — every family fitted and ranked, so "which
    morphology?" is answered by a number.
  - `FitResult` — the fitted profile, one-sigma parameter estimates, RMS
    residual, R², AIC/BIC, and `to_config_kwargs()` for a one-hop
    measured-trace-to-`AnalysisConfig` handoff.
  - `load_trace(path, scale=...)` — 2-column CSV with a pixel-to-mm
    factor.
  - `examples/16_fit_measured_trace.py` walks CSV → fit → knockdown.

  Two design choices deviate from the issue's literal wording, both
  because the literal version fails its own acceptance criterion:

  1. **Ranked by BIC, not by raw residual.** Three families carry four
     shape parameters and two carry three, and a four-parameter family
     can always match a three-parameter one at least as well. On data
     generated by `pure_sinusoidal` the residuals come out a tie
     (`triangular` 0.016670 against the true `pure` 0.016721 — a 0.3 %
     gap that is noise) and the raw ranking picks the wrong family. BIC
     charges for the extra parameters and recovers the generating family
     for all five. `criterion="rms"` gives the literal ordering.
  2. **Tilt is fitted jointly as a nuisance parameter, not subtracted
     beforehand.** A full-trace detrend removes the *mean*, pushing the
     far field negative where `A·exp(-dx²/w²) ≥ 0` cannot follow; on a
     Gaussian bump that costs RMS 0.0856 against 0.0043 for the joint
     fit. The two extra parameters cost every family equally, so the BIC
     ordering is unaffected. Seeding, unlike estimation, *is* taken from
     a detrended copy: under a 0.05 mm/mm tilt the raw seed puts the
     amplitude guess at twice the truth and the optimiser converges 55 %
     low.

  Scope guard, per the issue: this fits the *idealized families* to
  data. Meshing an arbitrary measured profile directly is out of scope —
  the fit residual is what tells a user when that is needed.

### Fixed
- Core — **`load_trace` read comma-separated files as a single column of
  `nan`, and a single-column file as a single row** (found while testing
  the above; both paths were new in the same change and never shipped).
  `numpy.genfromtxt(delimiter=None)` splits on whitespace only, so a CSV
  — the format the function advertises — parsed as one unusable column;
  the delimiter is now sniffed from the first data line. Separately,
  `np.atleast_2d` reshapes a one-column file into a one-*row* array,
  which passed the column check and then read `x` and `z` out of the
  same data; 1-D input is now disambiguated against the file's first
  line and rejected with the right message.
